...
export const validateApplyCoins = [
    ...
    body('coinsToUse').isInt({ min: 100 }).withMessage('Minimum 100 coins required'),
    handleValidationErrors
];
...
export const validateAddress = [
    ...
    body('street').trim().isLength({ min: 5, max: 200 }).withMessage('Street must be 5-200 characters'),
    body('city').trim().isLength({ min: 2, max: 50 }).withMessage('City must be 2-50 characters'),
    body('landmark').optional().isLength({ max: 200 }).withMessage('Landmark too long'),
...
export const validateFcmToken = [
    body('fcmToken').trim().isLength({ min: 10 }).withMessage('Invalid FCM token'),
    handleValidationErrors
];
...
export const validateAchievement = [
    body('achievementId').trim().isLength({ min: 1, max: 50 }).withMessage('Invalid achievement ID'),
    body('reward').optional().isInt({ min: 0 }).withMessage('Reward must be positive'),
    handleValidationErrors
];
